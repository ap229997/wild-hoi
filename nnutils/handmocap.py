...
def process_mocap_predictions_left(mocap_predictions, image, hand_wrapper=None, mask=None):
    if hand_wrapper is None:
        hand_wrapper = ManopthWrapper(side='left').to('cpu')
    one_hand = mocap_predictions[0]['left_hand']
    if one_hand is None:
        return None
    ...
    weak_cam = get_weak_camera(one_hand['pred_camera'], one_hand['bbox_top_left'], one_hand['bbox_scale_ratio'], hoi_bbox)
    ...
    data = {
        ...
        'betas': betas,
        'weak_cam': weak_cam,
        'pred_cam': one_hand['pred_camera'],
        'pred_vertices_img': one_hand['pred_vertices_img'],
        'pred_joints_img': one_hand['pred_joints_img'],
        'pred_vertices_smpl': one_hand['pred_vertices_smpl'],
        'pred_joints_smpl': one_hand['pred_joints_smpl'],
    }
    return data
...
def process_mocap_predictions(mocap_predictions, image, hand_wrapper=None, mask=None):
    ...
    # obj_bbox = image_utils.mask_to_bbox(mask, 'minmax')
    x1, y1 = one_hand['bbox_top_left'] 
    bbox_len = 224 / one_hand['bbox_scale_ratio'] # find out why/how 224/scale is used - because image is resized earlier to 224x224
    x2, y2 = x1 + bbox_len, y1 + bbox_len
...
def get_camera(pred_cam, hand_bbox_tl, bbox_scale, bbox, hand_wrapper, hA, rot, fx=10):
    ...
    cam, topleft, scale = image_utils.crop_weak_cam(
        pred_cam, hand_bbox_tl, bbox_scale, new_center, new_size) # compute cam for the bigger crop
    s, tx, ty = cam
    
    f = torch.FloatTensor([[fx, fx]]) # how to set fx?
    p = torch.FloatTensor([[0, 0]]) # what is this used for? - principal point of camera intrinsic

    translate = torch.FloatTensor([[tx, ty, fx/s]]) # offset to convert weak perspective camera to full perspective (Appendix A.1)
    
    _, joints = hand_wrapper(
        geom_utils.matrix_to_se3(geom_utils.axis_angle_t_to_matrix(rot)), 
        hA) # joints are transformed by the global orientation predicted by FrankMocap
    
    # print(translate.shape, joints.shape)
    cTh = geom_utils.axis_angle_t_to_matrix(
        rot, translate - joints[:, 5]) # transform into wrist coordinate frame
    return cTh, f, p


def get_weak_camera(pred_cam, hand_bbox_tl, bbox_scale, bbox):
    new_center = (bbox[0:2] + bbox[2:4]) / 2
    new_size = max(bbox[2:4] - bbox[0:2])
    cam, topleft, scale = image_utils.crop_weak_cam(
        pred_cam, hand_bbox_tl, bbox_scale, new_center, new_size) # compute cam for the bigger crop
