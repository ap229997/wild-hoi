...
class PixCoord(nn.Module):
    def __init__(self, cfg, z_dim, hA_dim, freq, **kwargs):
        super().__init__()
        self.cfg = cfg
        J = 16
        self.net = ImplicitNetwork(z_dim + J*3, multires=freq, **cfg.SDF, **cfg)
    ...
    def forward(self, xPoints, z, hA, cTx=None, 
                cam: PerspectiveCameras=None, jsTx=None, seg=None, multiview=None, nviews=None, **kwargs):
        N, P, _ = xPoints.size()
        ...
        outputs = {'soft_sdf': sdf_value}
        if self.cfg.OCCUPANCY:
            outputs['pred_occ_logits'] = sdf_value.squeeze(-1)
            pred_occ = torch.sigmoid(sdf_value) # 1 means not occupied (to keep consistent with [-1(inside),1(outside)] convention required by marching cubes)
            outputs['pred_occ'] = pred_occ.squeeze(-1)
            # outputs['og_sdf'] = sdf_value
            # outputs['soft_sdf'] = torch.sigmoid(sdf_value) # why is this required?
            sdf_value = (pred_occ>0.5).long() * 2.0 - 1.0

        return sdf_value, outputs
...
class ImplicitNetwork(nn.Module):
    def __init__(
        ...
        self.use_occupancy = kwargs.get('OCCUPANCY', False)
...
def build_net(cfg, z_dim=None, **kwargs):
    if z_dim is None:
        z_dim = cfg.Z_DIM
    if cfg.DEC == 'obj':
        dec = PixObj(cfg, z_dim, cfg.THETA_DIM, cfg.FREQ, **kwargs)
    else:
        dec = PixCoord(cfg, z_dim, cfg.THETA_DIM, cfg.FREQ, **kwargs)
    return dec
